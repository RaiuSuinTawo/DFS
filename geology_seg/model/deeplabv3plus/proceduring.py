...
__all__ = ['ResNet', 'resnet18', 'resnet34', 'resnet50','resnet_dual', 'load_model']
model_urls = {
...
class MyNet_add(nn.Module):
    ...
    def _forward_impl_2(self, x):
        ...
        return self.layer3(x) + self.conv3_1(self.maxpool2(x)), self.conv3_2(self.maxpool2(self.maxpool2(x)))
    
    def _forward_impl_3(self, x):
        x , x3_2 = self._forward_impl_2(x)
        #print('POOL ONCE:', x.shape) 256
        x = self.layer4(x)
        x = x + x3_2
        return x
...
def _resnet(arch, block, layers, pretrained, progress, out_set, **kwargs):
    ...
    model = MyNet_add(block, layers, out_set=out_set, **kwargs)
    if pretrained:
...
def resnet_dual(out_set=128, pretrained=False, progress=True, **kwargs):
    ...
    """
    return _resnet('resnet34', BasicBlock, [2, 2, 2, 2], pretrained, progress, out_set,
                   **kwargs)


def resnet34(pretrained=False, progress=0, **kwargs):
    r"""ResNet-34 model from
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/pdf/1512.03385.pdf>`_
