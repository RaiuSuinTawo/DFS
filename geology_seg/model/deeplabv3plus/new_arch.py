...
from .new_utils import mit_b0, mit_b1, mit_b2, mit_b3, mit_b4, mit_b5
...
class SegFormer_mod(nn.Module):
    def __init__(self, num_classes = 21, phi = 'b0', pretrained = False):
        super(SegFormer_mod, self).__init__()
        self.in_channels = {
    ...
    def forward(self, inputs):
        ...
        x = self.fuse(self.backbone1.forward(pos), self.backbone2.forward(neg))
        x = self.decode_head.forward(x)
        
        x = F.interpolate(x, size=(H, W), mode='bilinear', align_corners=True)
        return x
    
    def fuse(self, input1, input2):
        assert len(input1)==len(input2)
        ret = []
        for i in range(len(input1)):
            ret.append(input1[i]+input2[i])
        return ret
